...
We will draw several random samples of 8 observations ($n$) each *without* replacement 
...
So far, $n=8$. We now need a new simulation that adds a new step: we need to vary the size of *n*. (Remember our population size, *N*, is `r N`)
...
for (n in 9:20){
  ...
  for (i in 1:rep){
    ...
    #we first get a random sample of size n from the population
    sample <- sample(population, size=n, replace = FALSE)
    #and then calculate and store the sample mean
    sample_mean[i] <- mean(sample)
  }
  
  #finally, we plot the distribution of the 10,000 sample means for the relevant n
...
I function is composed by: i) body, ii) arguments, and iii) environment (usually Global unless otherwise specified)
